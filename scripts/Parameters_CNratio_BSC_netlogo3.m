...
%Assigning Biological factors
% Sp1 : PhotoTroph growth on CO2
% Sp2 : HeteroTrophs (aerobe)
% Sp3 : HeteroTrophs (anoxic)
% Sp4 : Nitrifier
%initMoles = 1;
...
%axiluN2 = NH4+
%H+ (pH measuering)
%EPS (CH2O: changes in diffusivity)
%sitesCCO3 = zeros(m,n);
%sitesCNH4 = zeros(m,n);
%sitesCH = zeros(m,n);
%sitesCEPS = zeros(m,n);
...
DiffusionList = [1.73 1.65 1.02 0.5184 1.47 1.41 1.64 1.5]*10^(-4)/(24*3600); %m^2/s %nitrate diffusion coefficient from Hirokawa
%[EPS CO3 NH3 H+ pH HONO Ca2+]
DiffusionAuxil = [0.1 0.79 1.41 8.04 0 1.47 0.864]*10^(-4)/(24*3600); %m^2/s  
...
germT   =   1/8; %days
% Same Mumax for all impemented as the highest mumax ecolie 1.25hr^-1;
%mumax = [30 30 30 30 30 30 30 30]/(24*60*60); %Maximum specific growth rate, unit: (s-1)
mumax = 3*[10 10 10 10 10 1.6 1.7 1]/(24*60*60); %Maximum specific growth rate, unit: (s-1) % AOB from bollmann et al
%mumax = [10 10 10 10 10 10 10 10]/(24*60*60); %Maximum specific growth rate, unit: (s-1)
mumax(end+1) = mumax(1)/10;
%mrate   =   [0.3 0.3 0.3 0.3 0.3 0.3 0.3 0.3]/(24*60*60);%maintenance rate, unit :per second % 1% of maximum grwoth rates
%mrate = mrateRatio*mumax;
mrate   =  3*[0.09 0.09 0.09 0.09 0.4 0.4 0.15 0.15]/(24*60*60);%maintenance rate, unit :per second
%mrate   =  [0.09 0.09 0.09 0.09 0.4 0.2 0.075 0.075]/(24*60*60);%maintenance rate, unit :per second
...
Molwt(9) = 23.7824; %C'1'H'0.1323'O'0.535'N'0.2198
...
%  
% (rc) Biomass half reaction : C(n)H(a)O(b)N(c) - Bacteria, Generic , N-Source : N2
% 0.3266 CO2 + 0.0359 N2 + 1 H+ + 1 e- --> 0.3266 C'1'H'0.1323'O'0.535'N'0.2198 + 0.4784 H2O [ ?G = 25.8916 KJ/e-eq ]
%  
% Energy reaction : 
% + 0.0417 C6H12O6 + 0.25 H2O + 0.1667 N2 + 0.3333 H+ --> + 0.25 CO2 + 0.3333 NH4+  
%  
% Synthesis reaction : 
% + 0.0417 C6H12O6 + 0.0766 CO2 + 0.0359 N2 --> 0.3266 C'1'H'0.1323'O'0.535'N'0.2198 + 0.2284 H2O  
%  
% Balanced equation using TEEM_2 : 
% [ fe = 0.86 ] [ fs = 0.14 ] [ e = 0.5 ]
% + 0.0417 C6H12O6 + 0.185 H2O + 0.1489 N2 + 0.288 H+ --> 0.0444 C'1'H'0.1323'O'0.535'N'0.2198 + 0.2056 CO2 + 0.288 NH4+  
%  
% Yield prediction :
% Yg/m = 25.337 [ grams_cells/mol_donor ]
% Yc/m = 1.065 [ mol_C_cells/mol_donor ]
% Yc/c = 0.178 [ mol_C_cell/mol_C_donor ]

a = [0.0417*6 0 0.185 0.1489 0.288 0.0444 00.2056 0.288];  
Y = a(6);
O2 = 0; %O2
...
Y = 0.5;
%Phototrophs(aerobes)  
A =[1 0 0 -1 0;2 1 3 -1 0; 0 2 0 -2 -1; 0 0 1 0 0;0 0 -1 0 1];
invA = inv(A);
%temp = [Y; Y+2; 2.5*Y; 0.17*Y; 0];
temp = [Y; 0.5350*Y+2; 0.1323*Y; 0.2198*Y; 0];
a = invA*temp;
%a1CO2 +a2H2O+a3NO3=YCH2.5ON0.17+a4CH2O+O2 +a5H+
O2 = Molwt(2)/Y; %O2
NH4 = 0; %NH4+
CO2 = -1*a(1)*Molwt(7)/Y; %CO2
H2O = -1*a(2)*Molwt(8)/Y; %H2O
CH2O = a(4)*Molwt(1)/Y;
HCO3 = 0;
NO3 =  -1*a(3)*Molwt(4)/Y;
N2 = 0;
...
%a1CO2 +a2H2O+a3NH4+ =YCH2.5ON0.17+a4CH2O+ O2 +a5H+
A =[1 0 0 -1 0;2 1 0 -1 0; 0 2 4 -2 -1; 0 0 1 0 0;0 0 1 0 -1];
invA = inv(A);
%temp = [Y; Y+2; 2.5*Y; 0.17*Y; 0];
temp = [Y; 0.5350*Y+2; 0.1323*Y; 0.2198*Y; 0];
a = invA*temp;
...
% %a1HCO3 +a2H2O+a3NO3- =YCH2.5ON0.17+a4CH2O+ O2 +a5H+
A =[1 2 0 -2 -1;1 0 0 -1 0; 3 1 3 -1 0; 0 0 1 0 0;1 0 1 0 1];
invA = inv(A);
%temp = [2.5*Y; Y; Y+2; 0.17*Y; 0];
temp = [0.1323*Y; Y; 0.5350*Y+2; 0.2198*Y; 0];
%temp = [Y; 0.5350*Y+2; 0.1323*Y; 0.2198*Y; 0];
a = invA*temp;
% %a1HCO3 +a2H2O+a3NO3- =YCH2.5ON0.17+a4CH2O+ O2 +a5H+
O2 = Molwt(2)/Y; %O2
...
A =[1 2 4 -2 -1;1 0 0 -1 0; 3 1 0 -1 0; 0 0 1 0 0;-1 0 1 0 -1];
invA = inv(A);
%temp = [2.5*Y; Y; Y+2; 0.17*Y; 0];
temp = [0.1323*Y; Y; 0.5350*Y+2; 0.2198*Y; 0];
a = invA*temp;
%a1HCO3- +a2H2O+a3NH4+ =YCH2.5ON0.17+a4CH2O+ O2 +a5H+
O2 = Molwt(2)/Y; %O2
NH4 = -1*a(3)*Molwt(11)/Y;  %NH4+
CO2 = 0; %CO2
H2O = -1*a(2)*Molwt(8)/Y; %H2O
...
% 0.3247 CO2 + 0.0915 HCO3- + 0.0915 NH4+ + 1 H+ + 1 e- --> 0.4162 C'1'H'0.1323'O'0.535'N'0.2198 + 0.7012 H2O [ ?G = 32.8977 KJ/e-eq ]
%  
% Energy reaction : 
% + 0.0417 C6H12O6 + 0.25 O2 --> + 0.25 CO2 + 0.25 H2O  
%  
% Synthesis reaction : 
% + 0.0417 C6H12O6 + 0.0747 CO2 + 0.0915 NH4+ + 0.0915 HCO3- --> 0.4162 C'1'H'0.1323'O'0.535'N'0.2198 + 0.4512 H2O  
%  
% Balanced equation using TEEM_2 : 
% [ fe = 0.5 ] [ fs = 0.5 ] [ e = 0.5 ]
% + 0.0417 C6H12O6 + 0.1256 O2 + 0.0455 NH4+ + 0.0455 HCO3- --> 0.2072 C'1'H'0.1323'O'0.535'N'0.2198 + 0.0884 CO2 + 0.3501 H2O  
%  
% Yield prediction :
% Yg/m = 118.237 [ grams_cells/mol_donor ]
% Yc/m = 4.972 [ mol_C_cells/mol_donor ]
% Yc/c = 0.829 [ mol_C_cell/mol_C_donor ]

a = [0.0417*6 0.1256 0.0455 0.0455 0.2072 0.0884 0.3501];
Y = a(5); 
...
H = a(4)/a(5);
CHxOyNz = Molwt(9);
%YphotoDecay = [CHxOyNz HCO3 CH2O NH4 H2O H]/Molwt(9);
...
H = a(4)/a(5);
CHxOyNz = Molwt(6);
%YheteroDecay = [CHxOyNz HCO3 CH2O NH4 H2O H]/Molwt(6);
%Ynitri2 = [O2 CO2 HCO3 CH2O NO3 NH4 N2 H2O H NO2 N2O]/Molwt(6);
...
% For autotrophe, simply assinged 10 /day for all needs to be changed based
% on literature values.
%Half saturation constant for all
%Ks = zeros(5,5); %row for species, column for nutrient
%Ks = [4.4 0.0744 -0.02 0 0;4.4 0 0.02 0 0;0 0 1.7*10^(-6) initMoles*iniConcentration(4) 2.729028;0 2.23 0 -initMoles*iniConcentration(4) 2.729028; 4.4 0 1.215 0.49918 0];
...
oxygenK = 5*0.2496;
%oxygenK = 5;
% Changed K_S values following Ribisco activity:20 \muM (bionumbers)
%Ks = [0.88 PhotoRespKO PhotoRespSugar 0 0 0.02;0 oxygenK 2.729028 0 0 1.7*10^(-6);0 -1*oxygenK 2.729028 0 0.0022 0; 0.88 0.49918 0 0 0 1.215];
...
Ks = [KsPh1;KsPh2;KsPh3;KsPh4;oxygenK 0 0.88 HeteroKs 0 1.7*10^(-6) 0;-1*oxygenK 0 0 HeteroKs 0.0022 0 0; 0.49918 0.88 0.88 0 0 0.9 0; 0.49918 0.88 0 0 0 0 4.920];%Nitrobactor's Ks value for NO2- has been used, Nitrosonma's minimum ammonium Km


%Ks = zeros(4,4);
%for i = 1:4
%Ks(:,i) = Ks1(:,i)./iniConcentration(i);
%end

% 
% %Half-rate constant in M-M equation, unit: (g/m3) or (mg/l) g/unitVolume
% % Negative implies inhibition term. Ks/(C+K_s)
% %----------------------------------------
% EPSfraction = 0.4; %25% of generated biomass from the growth mechanism will be retuned as EPS material
% hydroloysisEPS = 0.5; %becomes sugar
% inerGlucose = 0; %Inner mass of poly glucose : stored for dark respiration for phototrophs
% temp1 = -1*(EPSfraction+inerGlucose+1.0025)/(EPSfraction+inerGlucose+1.3409)*44.0095; %amount of CO2 for one mole of oxygen :Phototroph 1
% temp2 = -1*0.1704/(EPSfraction+inerGlucose+1)*17.03; %amount of NH3 for one mole of oxygen :Phototroph 2
% temp4 = 31.9988; %amount of oxygen g/m^3 : one mole
% temp3 = 1.0311*temp4/(EPSfraction+inerGlucose+1); %biomass of phtotroph 2 g.m^-3
% yieldPh1 = [temp1 temp4 0 0 0 temp2]/temp3;
% yieldPh1(3) = EPSfraction*hydroloysisEPS;
% yieldPh1(4) = EPSfraction*(1-hydroloysisEPS);
% yieldaero= [1.1915 -0.8013 -1.4775 0 0 -0.1383];
% yieldanoxic= [1.1915 0 -1.4775 0 -0.9401 0];
% yieldnutrifier= [-1.7870 -24.6225 0 0 25.1777 -7.0530];
% %yieldnutrifier= [-1.7870 -2.46225 0 0 2.51777 -0.70530];
% 
% Ymax = [yieldPh1; yieldaero;yieldanoxic;yieldnutrifier];
% YPG = 0.3;
% temp = [32*YPG/(1-YPG), -32/(1-YPG), 1-1.0025*YPG/(1-YPG), -0.1704/(1-YPG), -1];
% temp(1) = temp(1)*32.911089;
% temp(2) = temp(2)*24;
% temp(3) = temp(3)*44.0095;
% temp(4) = temp(4)*17.03;
% temp(5) = temp(5)*31.9988;
% temp = temp/(temp(1));
% Yrespi =[temp(3), temp(5), temp(2), 0, 0, temp(4)];
% YmaxCons = Ymax.*(Ymax<0);
% YmaxProd = Ymax.*(Ymax>0);
% 
% YrespiCons = Yrespi.*(Yrespi<0);
% YrespiProd = Yrespi.*(Yrespi>0);
