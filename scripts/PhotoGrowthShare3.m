function [apparentGrowth,respiGrowth] = PhotoGrowthShare3(sitesC, sitesC2,MumaxTt,intensityProfile,nutShare)
...
tempRespi = min(nutShare(:,:,1).*sitesC{1}./(Ks(1,1)+sitesC{1}),nutShare(:,:,6).*sitesN./(Ks(2,6)+sitesN)); %O2 and TAN
...
for i = 1:4 %all phototrohps
    ...
    compNH3 = (Ks(i,6)>=0).*nutShare(:,:,6).*(sitesN./(Ks(i,6)+sitesN)) + (Ks(i,6)<0).*(-1*Ks(i,6)./(-Ks(i,6)+sitesN));
