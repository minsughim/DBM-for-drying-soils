...
for indexT = 1:numberOfSamples
    ...
    cd(serial_id);
    %serial_idtemp = sprintf('Constant_light_bubble_Pa_1_5');
    %serial_idtemp = sprintf('Constant_dark_bubble_Pa_1_5');
     serial_idtemp = sprintf('Dinural_cycle_bubble_Pa_1_5');
  ...
  % serial_id2 = sprintf('HT_BSC_noPG_Tchange_Day%dPot%.1fpCO2frac%dNfix%.1fCat%.1favT%dPeneD%.1fredC%.2findex%d',examineDay,pot1,pCO2frac,NfixationR, amountCations, avgT, peneD, reducedLevelC, indexS);
    ...
    % save(strcat('./Results/',serial_id,'Dinural_Pa_2_essence.mat'));
