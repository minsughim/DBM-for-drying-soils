 function dy = HONO_forced_kinetics_Tdep_nitroacidium(y,Z,pKs, theta)
...
% Y10 = Ca 2+
% Y11 = H2ONO+
% Y12 = H + : written in y11

%% Dynamics
dy(11,1) = 0;%zeros(11,1);    % a column vector
...
protons = 0.5*(sqrt(totIon*totIon+4*Kaw)-totIon);

if protons < Kaw %apporoximation of the concentraiton of OH-
    frac = sqrt(totIon*totIon+4*Kaw);
else
    frac = Kaw/protons;
end

I = 0.5*((ions.*ions)*y+Z+protons+frac); %ionic strength
...
%rate
k1 = 2221/(24*60*60); %/s
reac1 = k1*(y(3) - y(4)*protons/K1c); %CO2 + H2O = HCO3- + H+
...
k3 = 10^10/(24*60*60); %/s
reac3 = k3*(y(4)-protons*y(5)/K2c);%HCO3- = H+ +CO32-
k4 = 10^10/(24*60*60); %/s
reac4 = k4*(y(1)-protons*y(2)/Ka);%NH4 = NH3 + H+
...
reac5 = k5*(y(7)-protons*y(6)/Kahono);%HNO2 = NO2- + H+
...
% Nitrous acid and nitroacidium reaction
k5 = 10^10/(24*60*60); %/s % Horokawa says 10^(6)s^(-1) almost 100 times faster than the value used here
reac8 = k5*(y(11)-protons*y(7)/Kah2ono);%H2ONO+ = HONO + H+
