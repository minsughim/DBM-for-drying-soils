function [sitesC2t,sitesC3, sitesC5, sitesC6,sitesC7, sitesC22,sitesC23, sitesC24,sitesC25 ,sitesC26, sitesC27, sitesCaTemp, sitesCnitroacidium] = PHestimationMex_nitroacidium(sitesC2t,sitesC3, sitesC5, sitesC6, sitesC7, sitesC22,sitesC23, sitesC24,sitesC25,sitesC26,sitesC27,sitesCaTemp, sitesCnitroacidium, DeltaT, ZionTemp,pKTotList,thetaTC)
%N1 = O2
%Carbon related
%N2 = CO2
%N3 = HCO3-
...
sitesTemp(:,:,11) = sitesCnitroacidium;
sitesTemp(:,:,12) = sitesC24; %H in M
...
y0 = zeros(11,1);
pKs = zeros(6,1);
ions = [1 0 0 -1 -2 -1 0 0 0 2 1];

for i = 1:m
    for j = 1:n
        pKs(:) = pKTotList(i,j,:);
        y0(:) = sitesTemp(i,j,1:11);
        ...
        try            
            [T2,Y2] = pH_mex_C_H2ONO(DeltaT, y0, [ZTemp(i,j) thetaTC(i,j) pKs'], [],[1e-8 1e-8 10]);
            solY = Y2(:,end);
        catch
            %warning(lasterr);
            [T,Y] = ode15s(@(t,y2) HONO_forced_kinetics_Tdep_nitroacidium(y2,ZTemp(i,j),pKs,thetaTC(i,j)), [0 DeltaT], y0,options);
            solY = Y(end,:)';
        end    
        ...
        protons = 0.5*(sqrt(totIon*totIon+4*10^(-14))-totIon);
        ...
        sitesC24(i,j) = protons;
        sitesC25(i,j) = -log10(protons);
        ...
        sitesCnitroacidium(i,j) = solY(11); %Ca in M
