...
%% Stoichiometry
%multiplytingF = 12.5; % Cyanobacteria has a higher carbon contents per cell compare to other bacterial cells 
multiplytingF = 1; % Cyanobacteria has a higher carbon contents per cell compare to other bacterial cells 
...
% temp = [cph*Y; oph*Y+2; hph*Y; nph*Y; 0];
% a = invA*temp;
...
% temp = [hph*Y; cph*Y; oph*Y+2; nph*Y; 0];
...
Y = 0.5;
%alphaCO = 2; %yield of C respected to the oxygen 
...
N2O = 0;
Yph1 = [O2 CO2 HCO3 CH2O NO3 NH4 NO2 N2O N2 H2O H]/Molwt(9);
maximumCfixRate = 6*10^(-18)*Molwt(7)/(mumax(1)*Vu*0.1*rho);
Yph1 = Yph1*maximumCfixRate/(abs(Yph1(2)));
...
Yph2 = [O2 CO2 HCO3 CH2O NO3 NH4 NO2 N2O N2 H2O H]/Molwt(9);
maximumCfixRate = 6*10^(-18)*Molwt(7)/(mumax(1)*Vu*0.1*rho);
Yph2 = Yph2*maximumCfixRate/(abs(Yph2(2)));
...
N2O = 0;
Yph3 = [O2 CO2 HCO3 CH2O NO3 NH4 NO2 N2O N2 H2O H]/Molwt(9);
maximumCfixRate = 6*10^(-18)*Molwt(10)/(mumax(1)*Vu*0.1*rho);
Yph3 = Yph3*maximumCfixRate/(abs(Yph3(3)));
...
Yph4 = [O2 CO2 HCO3 CH2O NO3 NH4 NO2 N2O N2 H2O H]/Molwt(9);
maximumCfixRate = 6*10^(-18)*Molwt(10)/(mumax(1)*Vu*0.1*rho);
Yph4 = Yph4*maximumCfixRate/(abs(Yph4(3)));
...
Ks = [KsPh1;KsPh2;KsPh3;KsPh4;oxygenK 0 0.0088 HeteroKs 0 1.7*10^(-6) 0;-1*oxygenK 0 0 HeteroKs 0.0022 0 0; 0.5 0.88 0.0088 0 0 0.9 0; 0.5 0.88 0 0 0 0 2.96];%Nitrobactor's Ks value for NO2- has been used, Nitrosonma's minimum ammonium Km
