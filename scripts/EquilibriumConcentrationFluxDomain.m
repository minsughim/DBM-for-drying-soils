function [efflux, sitesC, sitesCg, sitesC2,deltaNM] = EquilibriumConcentrationFluxDomain(waterVolume,gasVolume,sitesC,sitesC2,sitesCg,HenryDomain,InvasedIsland,AlphaM,sitesNEquil,changeN,iniConcentrationGas)

[m,n] = size(waterVolume);
efflux = zeros(1,5);
deltaNM = zeros(m,n,5);
ListGas = [1 2 6 7 8];
%oxygen, CO2, NH3
for iG =1:5
    ...
    if iN == 7 %HONO separately due to the index problem
        ...
        sitesCg{iN,1} = sitesCgtemp;
            ...
            totalN = sitesC2{3}.*waterVolume + sitesCg{iN,1}.*gasVolume;
            sitesCtemp = sitesNEquil{iN}.*InvasedIsland + (InvasedIsland==0).*HenryDomain{iN}.*totalN./AlphaM{iN};
            sitesCgtemp = iniConcentrationGas(iN).*InvasedIsland + (InvasedIsland==0).*totalN./AlphaM{iN};
            newTotN = sitesCtemp.*waterVolume + sitesCgtemp.*gasVolume;
            deltaN = -1*(newTotN - totalN).*InvasedIsland;
            ...
            totalN = sitesC{iN}.*waterVolume + sitesCg{iN,1}.*gasVolume;
            sitesCtemp = sitesNEquil{iN}.*InvasedIsland + (InvasedIsland==0).*HenryDomain{iN}.*totalN./AlphaM{iN};
            sitesCgtemp = iniConcentrationGas(iN).*InvasedIsland + (InvasedIsland==0).*totalN./AlphaM{iN};
            newTotN = sitesCtemp.*waterVolume + sitesCgtemp.*gasVolume;
            deltaN = -1*(newTotN - totalN).*InvasedIsland;
