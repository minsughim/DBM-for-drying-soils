function [apparentGrowth,respiGrowth] = PhotoGrowth3(sitesC,sitesC2, MumaxTt,intensityProfile)
...
tempRespi = min(sitesC{1}./(Ks(1,1)+sitesC{1}),sitesN./(Ks(2,6)+sitesN)); %O2 and TAN
...
for i = 1:4 %all phototrohps
    ...
    compNH3 = (Ks(i,6)>=0).*(sitesN./(Ks(i,6)+sitesN)) + (Ks(i,6)<0).*(-1*Ks(i,6)./(-Ks(i,6)+sitesN));
