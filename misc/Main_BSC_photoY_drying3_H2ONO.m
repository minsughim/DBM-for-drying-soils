function Main_BSC_photoY_drying3_H2ONO(NH3ppb, HONOppb, lightOn2,examineDay,desiccationIndex,pCO2frac, amountCations, avgT, alphaCO, pot1, plottt, indexS,maximumThreadt)
rng('shuffle')
...
cd ..
if lightOn2 == 1
    serial_idtemp = sprintf('Constant_light_H2ONO_NH3%d_HONO%d_Drying%d_varyingT', NH3ppb,HONOppb,desiccationIndex);
    amplitudeT = 0; %experiemnt with the constnat condition (with average velocity and maximum incidence of light)
        ...
        serial_idtemp = sprintf('Dinural_cycle_H2ONO_NH3%d_HONO%d_Drying%d_varyingT', NH3ppb,HONOppb,desiccationIndex);
        else
        serial_idtemp = sprintf('Dark_Tcycle_H2ONO_NH3%d_HONO%d_Drying%d_varyingT', NH3ppb,HONOppb,desiccationIndex);
        end
    end
end
mkdir(serial_idtemp);
...
sitesCnitroacidium = sitesH2ONOini*ones(m,n);
%sitesCa(1,:,1) = 0;
%sitesCa(end,:,1) = 0;
%sitesCa(1,:,2) = 0;
%sitesCa(end,:,2) = 0;
...
for examineT = 1:dataPoints
    ...
    for tt = 1:plottt
        ...
        if rem(t, (24*60*60/dt)) == 0; %every 6 hours save
        ...
        sitesC27 = sitesC2{7};
        sitesCaTemp = sitesCa;
        sitesCH2ONOTemp = sitesCnitroacidium;

        
        parfor iInd = 1:m
            [C2t, C3, C5, C6, C7, C22, C23, C24, C25, C26, C27, CaTemp, H2ONOTemp] = PHestimationMex_nitroacidium(sitesC2t(iInd,:),sitesC3(iInd,:), sitesC5(iInd,:), sitesC6(iInd,:),sitesC7(iInd,:), sitesC22(iInd,:),sitesC23(iInd,:), sitesC24(iInd,:),sitesC25(iInd,:),sitesC26(iInd,:),sitesC27(iInd,:), sitesCaTemp(iInd,:,:), sitesCH2ONOTemp(iInd,:), DeltaT, ZionM(iInd,:),pKTotList,thetaTC);       
            sitesC2t(iInd,:) = C2t;
            sitesC3(iInd,:) = C3;
            sitesC5(iInd,:) = C5;
            sitesC6(iInd,:) = C6;
            ...
            sitesCnitroacidium(iInd,:) = H2ONOTemp; 
        end
        sitesC{2} = sitesC2t;
        sitesC{3} = sitesC3;
        sitesC{5} = sitesC5;
