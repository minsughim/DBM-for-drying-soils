function Main_BSC_photoY_wetting3_conti(lightOn,examineDay,examineDay2, pCO2frac, amountCations, avgT, alphaCO, pot1, plottt, indexS,maximumThreadt)
rng('shuffle')
...
if lightOn == 1
    ...
    serial_idtemp = sprintf('Constant_light_conti');
    amplitudeT = 0; %experiemnt with the constnat condition (with average velocity and maximum incidence of light)
else if lightOn == 0
        serial_idtemp = sprintf('Constant_dark_conti');
        amplitudeT = 0; %experiemnt with the constnat condition (with average velocity and maximum incidence of light)
    else
        serial_idtemp = sprintf('Dinural_cycle_conti');
    end
end
mkdir(serial_idtemp);
cd ..
