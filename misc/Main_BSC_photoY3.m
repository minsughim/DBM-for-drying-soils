function Main_BSC_photoY3(examineDay, pCO2frac, amountCations, avgT, alphaCO, pot1, plottt, indexS,maximumThreadt)
rng('shuffle')
...
%maximumThreadt = 2;
%peneD = 0.2;
%alphaCO = 5; %yield of C respected to the oxygen 
...
if satuIndex == 0 % At field condition (water Saturation 0.66)-> not completely saturated case (amount of water held by capilarity only)
    serial_id = sprintf('HT_BSC_photoY3_Day%dPot%.1fpCO2frac%dCat%.1favT%dAlphaCO%dfindex%d',examineDay,pot1,pCO2frac,amountCations,avgT,alphaCO, indexS);
else
    serial_id = sprintf('HT_BSC_photoY3_satu_Day%dPot%.1fpCO2frac%dCat%.1favT%dAlphaCO%dfindex%d',examineDay,pot1,pCO2frac,amountCations,avgT,alphaCO, indexS);
end
...
%%%%%
m = 300; %Number of rows for top layer
n = 4; %Number of columns for top layer
LayerDepth = 0.02;
...
if satuIndex == 0 % At field condition (water Saturation 0.66)-> not completely saturated case (amount of water held by capilarity only)
    ...
    end    LocalWaterContents = waterFilm./totaldM;
    LocalWaterSaturation = LocalWaterContents./porosityM;
    LocalGasContents = porosityM-LocalWaterContents;
    Aqdiff = (LocalWaterContents.^(10/3)).*porosityM.^(-2);
    %Aqdiff = ones(m,n);
...
%MaxCapacityPop(:,5) = MaxCapacityPop(:,5)/1000;
...
%heterogeneous
%[intensityList,temperatureList,MumaxT, HenryConstList{1,1},HenryConstList{2,1},HenryConstList{6,1},HenryConstList{7,1},HenryConstList{8,1}, pKList] = EnvironmentProfile(depthList,timeLine,alphaMatrix);
...
for examineT = 1:dataPoints
    ...
    for tt = 1:plottt
        ...
        if rem(t, (12*60*60/dt)) == 0; %every 6 hours save
            ...
            serial_id2 = sprintf('HTBioCrustEPSCNcycleHour%d',exposedHours);
       ...
       % apparentGrowth(:,:,8) = WaterShortage.*apparentGrowth(:,:,8);
        ...
        %         %Boundary conditions for diffusion top and bottom of the domain.
        %         %Nitrogen deposition can be considered here.
        %         BClist = [iniConcentration(3) 0 iniConcentration(5) iniConcentration(6) iniConcentration(7)]; %HCO3- CH2O NO3- NH4+ NO2-
        %         BClist2 = [0 sitesC2ini{2} sitesC2ini{6} sitesC2ini{7}]; %EPS CO32-, HONO, Ca
        %         % BClist = [iniConcentration(3) 0 0 0 0]; %HCO3- CH2O NO3- NH4+ NO2-
        %         % BClist2 = [0 sitesC2ini{2} 0 sitesC2ini{7}]; %EPS CO32-, HONO, Ca
        %
        %         [sitesC,sitesCg, sitesC2, DeltaT,nutShare,totConsumed,changeN] = DiffusionProcess_CNratio_BSC3(Aqdiff,waterFilm,waterVolume,gasVolume,sitesC,sitesC2,sitesCg,HenryDomain,DiffusionList,DiffusionAuxil,reactionC,InvasedIsland,AlphaM,sitesNEquil,sitesNGEquil,listE,Lp,dt,reducedLevelC,BClist,BClist2);
        %
