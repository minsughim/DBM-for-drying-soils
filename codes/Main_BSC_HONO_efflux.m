function Main_BSC_HONO_efflux(examineDay, pot1, initMoles, indexS, maximumThreadt)
rng('shuffle')
...
if pot1 == 0 % for the completly saturated case
    serial_id = sprintf('HT_BSC_HONO_satu_Day%dCarbon%.1f_index%d',examineDay, initMoles, indexS);
    satuIndex = 1;
else
    serial_id = sprintf('HT_BSC_HONO_Day%d_Pot%.1fCarbon%.1f_index%d',examineDay,pot1,initMoles, indexS); 
    satuIndex = 0;
end
...
m = 128; %Number of rows for top layer
n = 64; %Number of columns for top layer
LayerDepth = 0.02; % 2cm
...
[sitesC2ini, sitesCaini, sitesCgini, sitesCini] = Main_single_noBio(PartialPressure,amountCations,avgT, initMoles);
...
%iniWalkersBiocrust_HONO %Same as biocurst but without photoautotrophs
iniWalkersBiocrust_HONO_random % without photoautotrophs, and random distribution of cells
...
sitesC2{1} = zeros(m,n); %EPS does not exist in this model (only soil and water)
%for i = 1:n
