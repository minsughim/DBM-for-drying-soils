function [efflux, sitesC, sitesCg, sitesC2] = EquilibriumConcentrationDensity(waterVolume,gasVolume,sitesC,sitesC2,sitesCg,HenryDomain,InvasedIsland,AlphaM,sitesNEquil,sitesNGEquil,changeN)
...
for iG =1:5
    ...
    if iN == 7 %HONO separately due to the index problem
        ...
        sitesCgtemp = sitesNGEquil{iN}.*InvasedIsland + (InvasedIsland==0).*totalN./AlphaM{iN};
        newTotN = sitesCtemp.*waterVolume + sitesCgtemp.*gasVolume;
        deltaN = (totalN- newTotN).*InvasedIsland;
