function [intensityList,temperatureList,MumaxT, HenryConstListO2,HenryConstListCO2,HenryConstListNH3, HenryConstListHONO, HenryConstListN2O, pKList] = ExperimentProfile(depthList,timeLine,alphaMatrix,lightOn)
...
for i = 1:tempT
    for j = 1:tempL
        ...
        tTemp = timeLine(i);
        dampD = dampingD(j);
        temperatureTemp = averageT + amplitudeT.*exp(-zTemp./dampD).*sin((2*pi*tTemp/24)+6-(zTemp./dampD));
        ...
        %constant light condition (not mimicking dinural cycle)
        if lightOn == 1
            intensityList(i,j) = MaximumIncidence.*exp(-zTemp/(attenuationRate));            
        else           
            intensityList(i,j) = 0;
        end
    end
